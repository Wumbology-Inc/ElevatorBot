...
class MainCommands():
    ...
    async def ver(self, ctx: commands.Context):
        """
        Reply with current ElevatorBot version number from the git master branch tag
        """
        currRepo = git.Repo('.')
    ...
    async def uptime(self, ctx: commands.Context):
        ...
        await ctx.send(f"ElevatorBot has been up for: {days}d {hours}h {minutes}m {seconds}s")
